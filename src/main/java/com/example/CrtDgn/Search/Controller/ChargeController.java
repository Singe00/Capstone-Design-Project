...
public class ChargeController {
    ...
    public List<Charge2> ChargePlace(@RequestBody List<ChargeDto> chargeDto){
        ...
        double[] distances = { 0.3, 0.6, 0.9, 1.2,1.5,1.8,2.1,2.4,2.7,3.0,300.0 }; // 주변 거리 범위 (단위: km)

        List<Charge2> cl = new ArrayList<>();
        List<Charge2> cl2 = new ArrayList<>();

        if (chargeDto.get(0).getOption() == 1)
        {
            ...
            for (Object[] row : result) {
                Charge2 charge2 = Charge2.builder()
                        ...
                        .traffic(((Integer) row[11]).toString()) // 혼잡도를 문자열로 변환하여 설정
                        .build();
                cl.add(charge2);
            }
        ...
        else if (chargeDto.get(0).getOption() == 2) {
